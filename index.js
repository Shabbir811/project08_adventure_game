...
//game variables: 
let enemies = ["skeleton", "zombies", "warrior", "assassin"]; // this is a array which containing our game's enemies names ;
let max_enemy_health = 85; //there we store maximum health of our game's enemy 85 health
let max_enemy_attack_demage = 25; // maximum damage when enemy  attack to our player , it's health is decrease 25 ;
//player variables:
let health = 100; //we store our player's health;
let max_attack_demage = 40; //maximum damage when our player attack to the enemy , enemy's health is decrease ;
let number_health_potion = 3; // this is like as lifes of our player. we give 3 chances to our player to increase health ;
let health_potion_heal_amount = 20; //every chance to increase player health by 20 ;
...
let running = true; // we declear a variable with initilzation boolean value true which we using in while loop;
console.log(`-`.repeat(60)); // this for formatting
console.log("---welcome to the shabbir's game---"); // this is intro of game;
Game: while (running) {
    console.log(`-`.repeat(60)); // this for formatting;
    let enemy = enemies[randomInt(enemies.length)]; // we select a random enemy from enemies array by using crypto module;
    let enemy_health = randomInt(max_enemy_health); // we set enemy's health randomly;
